...
class WallFollower:
    def __init__(self, dt: float) -> None:
        ...
        """
        self._dt: float = dt
        self._pred_dist = 0.2  # Desired distance from the wall
        ...
        self._safety_dist = 0.25  # Minimum safety distance to the front wall

        # STATE VARIABLES: Used to manage different navigation modes
        self._turn_left_state = False  # Indicates if the robot is turning left
        self._turn_right_state = False  # Indicates if the robot is turning right
        self._dead_end_state = False  # Activated when there’s no exit
        self._rotation = 0.0  # Tracks rotation progress during turns

        # LAST VALID DISTANCES: Used to handle NaN values from the sensor
        self._last_front_dist = 0.15  # Last valid front distance
        self._last_l_dist = 0.15  # Last valid left distance
        self._last_r_dist = 0.15  # Last valid right distance
    ...
    def compute_commands(self, z_scan: list[float], z_v: float, z_w: float) -> tuple[float, float]:
        ...
        if math.isnan(front_dist):
            front_dist = self._last_front_dist
        else:
            self._last_front_dist = front_dist

        if math.isnan(l_dist):
            l_dist = self._last_l_dist
        else:
            self._last_l_dist = l_dist

        if math.isnan(r_dist):
            r_dist = self._last_r_dist
        else:
            self._last_r_dist = r_dist
        ...
        if (
            front_dist <= self._safety_dist
            and l_dist <= 0.2
            and r_dist <= 0.2
        ):
            self._dead_end_state = False

        # 5. TURN DECISION: If an obstacle is detected ahead, determine the best turn direction
        if front_dist <= self._safety_dist and not self._dead_end_state and not self._turn_left_state and not self._turn_right_state:
            if r_dist >= l_dist:  # Turn towards the more open side
                self._turn_right_state = True
            else:
                self._turn_left_state = True

        # 6. RIGHT TURN MODE: Turns the robot right when an obstacle is in front
        if self._turn_right_state:
            v = 0.0  # Stop moving forward while turning
            w = 1.0  # Rotate to the right
            self._rotation += abs(w) * self._dt  # Track rotation progress

            if self._rotation >= math.pi / 4:
                self._turn_right_state = False
                self.last_error = 0  # Reset PID errors
                self.integral_error = 0
                self._rotation = 0.0  # Reset rotation tracker
            return v, w

        # 7. LEFT TURN MODE: Turns the robot left when an obstacle is in front
        elif self._turn_left_state:
            v = 0.0  # Stop moving forward while turning
        ...
        # 9. WALL FOLLOWING MODE: If no obstacles, adjust trajectory using PID control
            ...
            if r_dist >= l_dist:
                error = l_dist - self._pred_dist  # Error is the difference from desired distance
            else:
                error = self._pred_dist - r_dist

            # 10. PID CONTROL CALCULATION: Adjust angular velocity based on distance error
            derivative_error = (error - self.last_error) / self._dt  # Change in error over time
            self.integral_error += error * self._dt  # Accumulate integral error
